...
const STORAGE_PEER_KEY = 'chiikawa_my_peer_id';
const STORAGE_LAST_TARGET_KEY = 'chiikawa_last_target_id';
const STORAGE_CANVAS_KEY = 'chiikawa_canvas_drawing';

// 내 고유 ID 유지
let savedMyId = localStorage.getItem(STORAGE_PEER_KEY);
if (!savedMyId) {
  savedMyId = 'chiikawa-' + Math.random().toString(36).substring(2, 6);
...
function setupConnection(c) {
  ...
  conn.on('open', () => {
    status.className = 'status-on'; // 수정된 변수 반영
  ...
  conn.on('close', () => {
    ...
    if (lastTarget) {
      setTimeout(() => {
        if (!conn || !conn.open) connectToPeer(lastTarget);
      }, 3000);
    }
  });
}
