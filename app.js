...
function sendMyToken() {
  const tokenToSend = myFcmToken || localStorage.getItem('usagi_my_fcm_token');
  if (conn && conn.open && tokenToSend) {
    conn.send({
      type: 'token_exchange',
...
function bindConnectionEvents(c) {
  ...
  conn.on('data', (data) => {
    if (data.type === 'draw') {
      ...
      saveCanvasToStorage();
    } else if (data.type === 'token_exchange' || data.type === 'sync_init' || data.type === 'sync_full') {
      if (data.fcmToken) {
...
// 🔔 호출 버튼 (앱 켜짐: P2P 호출 / 앱 꺼짐: FCM 백그라운드 푸시)
