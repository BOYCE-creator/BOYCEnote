...
// URL-safe Base64를 Uint8Array로 변환 (iOS VAPID 필수)
    ...
    if (permission === 'granted') {
      ...
      if ('pushManager' in reg) {
        try {
          ...
          if (!sub) {
            // ntfy 공개 VAPID 키 사용
            const ntfyVapidKey = 'BNv6N_8C7rN91jU2g0bVp-mZ9Yx0c7g7J3rY_h0Ym4o8F7z6s7R9w8x3p1q4w5e6r7t8y9u0i1o2p3a4s5d6f7g';
            sub = await reg.pushManager.subscribe({
              userVisibleOnly: true,
              applicationServerKey: urlBase64ToUint8Array(ntfyVapidKey)
            });
          }

          // ntfy 서버에 내 토픽(usagi_내코드)으로 Web Push 엔드포인트 연동
          await fetch(`https://ntfy.sh/usagi_${myShortId}`, {
            method: 'POST',
            headers: { 'Title': '우사기 알림 활성화' },
            body: '이제 아이폰에서도 앱을 닫은 상태로 상단 알림이 정상 작동합니다!'
          });
        } catch (subErr) {
          console.warn('Push subscription init:', subErr);
        }
      }

      reg.showNotification('우사기 알림 활성화', {
        body: '이제 앱을 닫아도 백그라운드 상단 알림이 수신됩니다!',
        icon: './icon.png',
        vibrate: [200, 100, 200]
      });
      ...
      alert('알림 권한이 거부되었습니다. [설정] > [우사기]에서 알림을 허용해 주세요.');
    }
  } catch (err) {
...
// 🔔 호출 버튼 (포그라운드 P2P + 백그라운드 Web Push 동시 발송)
callBtn.addEventListener('click', async () => {
  ...
  const targetCode = targetCodeInput.value.trim() || localStorage.getItem(STORAGE_SAVED_PEER_KEY);
  
  if (targetCode) {
    try {
      await fetch(`https://ntfy.sh/usagi_${targetCode}`, {
        method: 'POST',
        headers: {
          'Title': '우사기 호출',
          'Priority': 'urgent',
