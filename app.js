...
function bindConnectionEvents(c) {
  ...
  conn.on('open', () => {
    ...
    // 연결되자마자 내 기기에 저장된 최신 그림과 토큰을 상대에게 전송
  ...
  conn.on('data', (data) => {
    if (data.type === 'draw') {
      ...
      // 상대가 보낸 그림이 있을 때만 화면 갱신 및 로컬스토리지 저장
      ...
      if (permission === 'granted') {
        if (typeof firebase !== 'undefined' && firebase.messaging) {
          ...
          if (token) {
            ...
            if (conn && conn.open) {
              conn.send({ type: 'sync_full', image: localStorage.getItem(STORAGE_CANVAS_KEY), fcmToken: myFcmToken });
            }
          }
        }
