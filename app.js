// ==========================================
// 🔔 Firebase 웹 푸시(FCM) 초기화 (실제 인증 키 적용)
// ==========================================
...
koreaClockEl.addEventListener('click', async () => {
  ...
  try {
    ...
    if (permission === 'granted') {
      ...
      alert('알림 권한이 거부되었습니다. 사이트/앱 설정에서 허용으로 변경해 주세요.');
    }
  } catch (err) {
...
// 🔔 호출 버튼 (실시간 P2P 전송 + 백그라운드 FCM 푸시 발송)
    ...
    try {
      await fetch('https://fcm.googleapis.com/fcm/send', {
        ...
        headers: {
          ...
          'Authorization': 'key=AIzaSyAISZZEHQSiKCjAN3ACqfgq710mOmpxX_Q'
        },
        body: JSON.stringify({
          to: targetToken,
          priority: 'high',
