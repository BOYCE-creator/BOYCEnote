// ==========================================
// 🔔 Firebase 웹 푸시(FCM) 초기화 (아이폰 완벽 호환)
// ==========================================
...
// 🔔 시계 터치 시 iOS/Android 무조건 권한 요청 실행
...
koreaClockEl.addEventListener('click', async () => {
  ...
  try {
    ...
    if (permission === 'granted') {
      ...
      alert('알림 권한이 차단되어 있습니다. [설정] > [알림]에서 우사기 알림을 허용해 주세요.');
    } else {
      alert('알림 권한 요청이 취소되었습니다. 다시 터치해 주세요.');
    }
  } catch (err) {
    console.error('FCM Error:', err);
    alert('오류 내용: ' + (err.message || err));
  }
});
...
// 🔔 호출 버튼 (포그라운드 실시간 호출 + 백그라운드 푸시 전송)
    ...
    try {
      await fetch('https://fcm.googleapis.com/fcm/send', {
        ...
        headers: {
          ...
          'Authorization': 'key=AIzaSyAISZZEHQSiKCjAN3ACqfg1Y5y8Z3M3e_w'
        },
        body: JSON.stringify({
          to: targetToken,
          priority: 'high',
