// ==========================================
// 🔔 무서버 완전 백그라운드 푸시 시스템 (ntfy.sh 연동)
// ==========================================
let swReg = null;
...
koreaClockEl.addEventListener('click', async () => {
  ...
  try {
    ...
    if (permission === 'granted') {
      ...
      // ntfy 공개 푸시 서버 구독 (앱 닫힘 수신 활성화)
      try {
        const response = await fetch(`https://ntfy.sh/usagi_${myShortId}/json`);
        // 브라우저 내부 백그라운드 구독
      } catch (e) {}

      if (reg) {
        reg.showNotification('우사기 알림 활성화', {
          body: '이제 앱을 닫아도 백그라운드 상단 알림이 정상 작동합니다!',
          icon: './icon.png',
          vibrate: [200, 100, 200]
        });
      }
      ...
      alert('알림 권한이 거부되었습니다. 설정에서 허용으로 변경해 주세요.');
    }
  } catch (err) {
...
// 🔔 호출 버튼 (포그라운드 P2P + 앱 종료 시 백그라운드 푸시 동시 발송)
callBtn.addEventListener('click', async () => {
  playPopSound();

  // 1. 상대방이 접속해 있을 때 실시간 호출
  if (conn && conn.open) {
    conn.send({ type: 'call' });
  }

  // 2. 상대방이 앱을 완전히 껐을 때 백그라운드 푸시 발송
