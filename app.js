...
const STORAGE_CANVAS_KEY = 'usagi_drawing_data_final_cloud';
const STORAGE_TIME_KEY = 'usagi_drawing_time_final_cloud';
...
const firebaseConfig = {
  ...
  databaseURL: "https://usagi-app-23849-default-rtdb.firebaseio.com"
};
...
// ☁️ 클라우드 DB 실시간 연결 및 즉시 동기화
function attachCloudRoom(targetId) {
  if (!db || !targetId || !myShortId) return;
  const roomId = getRoomId(myShortId, targetId);
  if (!roomId) return;

  if (roomRef) {
    roomRef.off();
  }
  roomRef = db.ref('drawings/' + roomId);

  roomRef.on('value', (snapshot) => {
    // 내가 지금 선을 긋고 있는 도중에는 화면 덮어쓰기 방지 (끊김 완벽 차단)
    if (isDrawing) return;

    const data = snapshot.val();
    if (data && data.image !== undefined) {
      const myLastTime = parseInt(localStorage.getItem(STORAGE_TIME_KEY) || '0', 10);
      if (data.updatedAt >= myLastTime) {
        if (data.image === '') {
          ctx.clearRect(0, 0, canvas.width, canvas.height);
          localStorage.setItem(STORAGE_CANVAS_KEY, '');
          localStorage.setItem(STORAGE_TIME_KEY, data.updatedAt.toString());
        } else {
          const img = new Image();
          img.onload = () => {
            ctx.clearRect(0, 0, canvas.width, canvas.height);
...
// 앱 시작 시 즉시 클라우드 DB 동기화
const savedTargetOnInit = localStorage.getItem(STORAGE_SAVED_PEER_KEY);
if (savedTargetOnInit) {
  targetCodeInput.value = savedTargetOnInit;
  attachCloudRoom(savedTargetOnInit);
}
...
// 그림 저장 (손을 뗐을 때만 호출되어 렉과 끊김 방지)
function saveCanvasToStorage(customTime = null, isCleared = false) {
  try {
    const dataUrl = isCleared ? '' : canvas.toDataURL();
...
function initPeer() {
  ...
  peer.on('error', (err) => {
    ...
    if (err.type === 'peer-unavailable') {
      setTimeout(() => {
        const savedTarget = localStorage.getItem(STORAGE_SAVED_PEER_KEY);
        if (savedTarget) tryAutoConnect(savedTarget);
      }, 2000);
    }
  });
...
function bindConnectionEvents(c) {
  ...
  conn.on('data', (data) => {
    ...
    if (data.type === 'draw') {
      // 상대방의 선을 부드럽게 실시간 렌더링
      drawLine(data.x0, data.y0, data.x1, data.y1, data.color, data.size, data.isEraser, false);
    } else if (data.type === 'stroke_end') {
      // 상대방이 선 그리기를 마쳤을 때만 최종 저장
      localStorage.setItem(STORAGE_TIME_KEY, (data.time || Date.now()).toString());
    } else if (data.type === 'sync_init') {
      if (data.fcmToken) {
      ...
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      localStorage.setItem(STORAGE_CANVAS_KEY, '');
      localStorage.setItem(STORAGE_TIME_KEY, (data.updatedAt || Date.now()).toString());
      playYahaSound();
...
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible') {
    ...
    if (savedTarget) {
      ...
      if (!conn || !conn.open) tryAutoConnect(savedTarget);
    }
  }
});
...
window.addEventListener('touchend', () => {
  if (isDrawing) {
    isDrawing = false;
    const { time } = saveCanvasToStorage();
    if (conn && conn.open) {
      conn.send({ type: 'stroke_end', time });
    }
  }
}, { passive: false });
...
clearBtn.addEventListener('click', () => {
  if (confirm('그림을 전부 지울까요? (야하-!)')) {
    playYahaSound();
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    const { time } = saveCanvasToStorage(null, true);
...
disconnectBtn.addEventListener('click', () => {
  ...
  if (confirm('친구와의 연결을 끊을까요?')) {
    ...
    localStorage.removeItem(STORAGE_CANVAS_KEY);
