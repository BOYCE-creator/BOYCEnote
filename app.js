// ==========================================
// 🔔 Firebase 웹 푸시(FCM) 초기화 (iOS 권한 대응)
// ==========================================
...
// 아이폰(iOS) 전용: 사용자 첫 터치 시 권한 팝업 강제 실행
function requestNotificationOnce() {
  if (typeof Notification === 'undefined') return;
  if (Notification.permission === 'default' && messaging && serviceWorkerReg) {
    Notification.requestPermission().then((permission) => {
      if (permission === 'granted') {
        messaging.getToken({ vapidKey: VAPID_KEY, serviceWorkerRegistration: serviceWorkerReg })
          .then((token) => {
            myFcmToken = token;
            if (conn && conn.open) {
              conn.send({ type: 'sync', image: canvas.toDataURL(), fcmToken: myFcmToken });
            }
          }).catch(console.warn);
      }
    });
...
window.addEventListener('click', requestNotificationOnce, { once: true });
window.addEventListener('touchstart', requestNotificationOnce, { once: true });
