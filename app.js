...
function attachCloudRoom(targetId) {
  ...
  // 앱 켜자마자 상대방이 혼자 그리고 나간 그림 즉시 수신
...
// 🌟 앱 켜지자마자 저장된 친구 코드로 클라우드 DB 즉시 동기화
...
function bindConnectionEvents(c) {
  ...
  conn.on('data', (data) => {
    ...
    if (data.type === 'draw') {
      ...
      saveCanvasToStorage(data.timestamp || Date.now());
      ...
      if (peerTime >= myLastTime) {
        ...
        if (myDrawing && myDrawing.length > 50) {
          conn.send({
            type: 'sync_full',
            image: myDrawing,
            updatedAt: myLastTime
      ...
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      saveCanvasToStorage(data.updatedAt || Date.now(), true);
      playYahaSound();
