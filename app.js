...
function bindConnectionEvents(c) {
  ...
  conn.on('open', () => {
    ...
    // 연결 시 내 그림 데이터와 최종 수정 시각을 상대에게 전송
    conn.send({
      type: 'sync_init',
  ...
  conn.on('data', (data) => {
    if (data.type === 'draw') {
      ...
      // 상대방이 실시간으로 그릴 때도 내 폰의 타임스탬프를 갱신
      ...
      // 🌟 상대방이 더 최근에 작업했을 때만 내 화면을 상대 그림으로 교체
      if (peerTime >= myLastTime) {
        ...
        // 🌟 내가 더 최근에 그렸거나 지웠다면, 내 화면으로 상대방 화면을 강제 덮어쓰기
