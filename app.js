...
const STORAGE_CANVAS_KEY = 'usagi_drawing_data_final_v3';
const STORAGE_TIME_KEY = 'usagi_drawing_time_final_v3';
...
// 캔버스 초기화 (크기 변경 시 그림 초기화 버그 완벽 차단)
function initCanvas() {
...
function bindConnectionEvents(c) {
  ...
  conn.on('data', (data) => {
    if (data.type === 'draw') {
      ...
      // 🌟 상대방이 더 최근에 작업했거나 내가 그린 게 없으면 화면을 교체
