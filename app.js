...
const STORAGE_MY_CODE_KEY = 'usagi_my_short_id';

// 4자리 짧은 고유 코드 생성/유지 (예: usagi-8a2f)
let myShortId = localStorage.getItem(STORAGE_MY_CODE_KEY);
if (!myShortId || myShortId.length > 15) {
  myShortId = 'usagi-' + Math.random().toString(36).substring(2, 6);
  localStorage.setItem(STORAGE_MY_CODE_KEY, myShortId);
}
...
disconnectBtn.addEventListener('click', () => {
  playPopSound();
  if (confirm('친구와의 연결을 끊을까요? (자동 재연결도 해제됩니다)')) {
