...
// 그림 로컬 & 클라우드(DB) 동시 저장
function saveCanvasToStorage(customTime = null) {
  try {
    ...
    // ☁️ Firebase 중앙 DB에 실시간 백업 (둘 다 나가도 유지)
    if (roomRef) {
      roomRef.set({
        image: dataUrl,
        updatedAt: now,
...
function attachCloudRoom(targetId) {
  ...
  // ☁️ 둘 다 꺼진 상태에서 나중에 혼자 들어와도 최신 그림을 서버에서 자동 다운로드
...
function bindConnectionEvents(c) {
  ...
  conn.on('data', (data) => {
    ...
    if (data.type === 'draw') {
      ...
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      saveCanvasToStorage(data.updatedAt || Date.now());
      playYahaSound();
