...
const STORAGE_SHORT_ID_KEY = 'usagi_clean_short_id_v5';
...
if (myCodeEl) myCodeEl.innerText = myShortId;

// ==========================================
// 2. Cloudflare 푸시 서버 및 Firebase 연동
// ==========================================
const PUSH_RELAY_SERVER_URL = 'https://royal-shape-3ed3.swc4876-f90.workers.dev';
    ...
    if (permission === 'granted') {
      if (typeof firebase !== 'undefined' && firebase.messaging) {
        ...
        if (token) {
          ...
          if (conn && conn.open) {
            conn.send({ type: 'sync_full', image: canvas.toDataURL(), fcmToken: myFcmToken });
          }
        }
      }
...
function drawLine(x0, y0, x1, y1, color, size, eraser, emit = true) {
  ...
  if (emit && conn && conn.open) {
    conn.send({ type: 'draw', x0, y0, x1, y1, color, size, isEraser: eraser });
  }
}
...
window.addEventListener('touchend', () => {
  if (isDrawing) {
    isDrawing = false;
    const currentData = saveCanvasToStorage();
    if (conn && conn.open && currentData) {
      conn.send({ type: 'sync_full', image: currentData });
    }
  }
}, { passive: false });
...
function initPeer() {
  ...
  peer.on('open', (id) => {
    if (myCodeEl) myCodeEl.innerText = id;
...
function bindConnectionEvents(c) {
  ...
  conn.on('open', () => {
    ...
    const currentDrawing = saveCanvasToStorage();
    conn.send({
      type: 'sync_full',
...
// 🔔 호출 버튼 (실시간 P2P + 앱 꺼져있을 때 Cloudflare 중계 서버를 통한 FCM 24시간 푸시)
